...
class RenderCards {
	public static function cards( $projects, $settings = array() ) {
		...
		$manual_fields = array_filter( array_map( 'sanitize_key', preg_split( '/[,\n]+/', (string) ( $settings['manual_card_fields'] ?? '' ) ) ) );
		foreach ( $projects as $index => $project ) {
			$id = $project->ID;
			$image = get_the_post_thumbnail_url( $id, 'large' );
			if ( ! $image ) { $gallery = get_gallery_ids( $id ); $image = $gallery ? wp_get_attachment_image_url( $gallery[0], 'large' ) : ''; }
			...
			foreach ( $field_keys as $field_key ) {
				...
				if ( '' !== $text ) $card_values[] = array( 'label' => $data['label'] ?? '', 'value' => $text, 'key' => $field_key );
			}
			$index_no = sprintf( '%02d', ( (int) $index + 1 ) );
			$classes = 'pss-card pss-card--' . esc_attr( $preset ) . ' pss-card--anim-' . esc_attr( $animation );
			...
			if ( $image ) $html .= '<img loading="lazy" src="' . esc_url( $image ) . '" alt="' . esc_attr( get_the_title( $id ) ) . '">';
			$html .= '<span class="pss-card__veil"></span><span class="pss-card__index">' . esc_html( $index_no ) . '</span><span class="pss-card__arrow">↗</span><span class="pss-card__field-stack">';
			...
			$html .= '<span class="pss-card__eyebrow">' . esc_html( $style ?: 'Project' ) . '</span>';
			...
			if ( ! empty( $card_values ) ) {
				...
				foreach ( array_slice( $card_values, 0, 4 ) as $field ) $html .= '<span><small>' . esc_html( $field['label'] ) . '</small><b>' . esc_html( $field['value'] ) . '</b></span>';
				$html .= '</div>';
			}
			$html .= '</div></a></article>';
		}
