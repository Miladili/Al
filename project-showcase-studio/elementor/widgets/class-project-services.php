...
class Project_Services extends Base {
	public function get_name() { return 'pss_project_services'; }
	public function get_title() { return 'Project Services'; }
	...
	protected function register_controls() {
		...
		$this->add_control( 'field_key', array(
			...
			'description' => 'Leave empty to use the built-in Project Services field.',
		) );
	...
	protected function render() {
		...
		$value = $key ? \PSS\get_field_value( $project_id, $key, '' ) : \PSS\get_meta( $project_id, '_pss_services', '' );
		$text = \PSS\field_value_text( $value );
		if ( '' === trim( $text ) ) return;
		$style = sanitize_key( $settings['style'] ?? 'clean' );
		echo '<div class="pss-project-simple-widget pss-project-services pss-project-services--' . esc_attr( $style ) . '">';
		echo '<span class="pss-project-simple-widget__label">Services</span>';
