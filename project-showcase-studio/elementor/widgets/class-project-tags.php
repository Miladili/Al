...
class Project_Tags extends Base {
	...
	protected function register_controls() {
		...
		$this->add_control( 'taxonomies', array(
			'label' => 'Taxonomies',
			'type' => \Elementor\Controls_Manager::SELECT,
			'default' => 'all',
			'options' => array( 'all' => 'All Project Taxonomies', 'category' => 'Category', 'style' => 'Style', 'location' => 'Location', 'type' => 'Project Type' ),
		) );
		$this->add_control( 'animation', array(
			'label' => 'Animation',
			'type' => \Elementor\Controls_Manager::SELECT,
			'default' => 'soft',
			'options' => array( 'none' => 'None', 'soft' => 'Soft', 'lift' => 'Lift' ),
		) );
	...
	protected function render() {
		...
		foreach ( $taxes as $taxonomy ) {
			...
			foreach ( $list as $term ) $terms[] = $term->name;
		}
		...
		foreach ( $terms as $term ) echo '<span>' . esc_html( $term ) . '</span>';
		echo '</div>';
	}
}
